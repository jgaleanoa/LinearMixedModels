...
\section{Ejercicios 2 y 5}

**2)** Calcule estadísticos de resumen de acuerdo a la variable género
y a la variable tiempo. Interprete. 

**5)** Obtenga boxplots de acuerdo al tiempo y al género. Interprete.
...
```{r}
# Se define un vector de colores
...
\subsection{Comparacion de medias}
...
```{r echo=F}
kable(mean.data, booktabs = T, align = "c") %>% 
  kable_styling(latex_options = "HOLD_position")
```
...
ggplot(df, aes(x = Tiempo, y = Genero, fill = Genero)) +
  ...
  labs(title = "Densidad por genero", x = "Tiempo", y = "Genero")
```
...
ggplot(df, aes(x = factor(Experimento), y = Tiempo, fill = Genero)) +
  ...
  geom_line(alpha = 0) +
  stat_summary(fun = mean, aes(group = Genero, col = Genero), geom = "line", size = 2)+
...
Claramente existe una diferencia entre los boxplot. Los boxplot de los hombres se encuentran ligeramente por debajo del de las muejeres. indicando que estos caminan mas rapido. Ahora, esto tambien se puede evidenciar al analizar el grafico de medias (puntos amarillos). Esto representa la media de tanto hombres, como mujeres; en cada experimento. De esta forma, la media de los hombres se encuentra debajo de las mujeres.
...
Parece haber alguna diferencia en los tiempos registrados por mujeres y hombres?
